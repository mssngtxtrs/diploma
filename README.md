...
При импорте схемы базы данных автоматически создаётся стандартная учётная запись администратора:
...
> **Важно:** сразу после первого входа смените пароль на надёжный. Использование стандартного пароля в production-окружении представляет угрозу безопасности.
 
Администратор имеет доступ к панели управления по адресу `/admin`, а также через кнопку в шапке сайта (отображается только для учётных записей с правами администратора). В админ-панели доступно управление заявками, тарифами, серверами и пользователями.
 
### 5.2. Регистрация пользователей
 
Обычные пользователи регистрируются через форму на странице `/auth`. После регистрации им становится доступен личный кабинет (`/dashboard`) для управления своими заявками.
